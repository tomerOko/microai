...
docker_build("signup", "services/signup", live_update=[sync("services/signup/src", "/app/src"), sync("services/signup/.vscode", "/app/nothing")]) # this line tells tilt to build the docker image of the signup service and to monitor the src folder for changes (basically, 'build it, rebuild in case of changes, specifically in the src folder just inject the changes to avoid unnecessary rebuild for every code change')
