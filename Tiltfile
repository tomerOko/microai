# this line tells tilt to apply the k8s/signup-d.yaml file and to monitor it (simply put, it tells tilt 'this file is under your responsibility')
k8s_yaml("k8s/signup-d.yaml") 
# this line tells tilt to build the docker image of the signup service and to monitor the src folder for changes (basically, 'build it, rebuild in case of changes, specifically in the src folder just inject the changes to avoid unnecessary rebuild for every code change')
...
# this line tells tilt to create a port-forward from the machine to the containers (used for: attaching the debugger, testing the service, connecting to rabbitmq if the services run outside the cluster, etc.)
